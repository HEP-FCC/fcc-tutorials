...
void plot_primary_vertex() {
  gROOT->Reset();

  TFile* infile = new TFile("primary_Zuds.root");
  ...
  // Plot the normalized chi2 / ndf :
  TH1F* h_chi2 = new TH1F("h_chi2", ";chi2; Events", 100, 0., 10.);
  events->Draw("PrimaryVertex.chi2 >> h_chi2");
  ...
  c1->Print("pv_resolutions.png");
  c1->Print("pv_resolutions.pdf");
  ...
  c2->Print("pv_pulls.png");
  c2->Print("pv_pulls.pdf");
}
